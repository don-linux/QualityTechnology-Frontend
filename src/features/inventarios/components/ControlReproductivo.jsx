...
const ControlReproductivo = () => {
  ...
  const cargarPiletasReproductoras = useCallback(async () => {
    try {
      ...
      console.error("Error cargando piletas control reproductivo:", err);
    }
  }, []);
  ...
  const cargarRegistros = useCallback(async () => {
    try {
      ...
      console.error("Error cargando registros de control reproductivo:", err);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    cargarRegistros();
  }, [cargarPiletasReproductoras, cargarRegistros]);
  ...
  const registrarControlReproductivo = async () => {
  ...
  const actualizarControlReproductivo = async () => {
    if (!validate(formData, requiredFields)) return;
    if (Number(formData.cantidad_total || 0) < 1) {
      showSnackbar("La cantidad total debe ser mayor a cero.", "error");
      return;
  ...
  const eliminarControlReproductivo = async (id) => {
    ...
    try {
      ...
      console.error("Error al eliminar control reproductivo:", err);
        ...
        <Card sx={{ mb: 5, borderRadius: 3, boxShadow: 3, bgcolor: "#fff" }}>
          <CardContent sx={{ p: { xs: 2, sm: 3 } }}>
            <Typography variant="h6" sx={{ mb: 3, fontWeight: 700, color: "#1a3c34" }}>
              {modoEdicion ? "Editar registro" : "Registrar nuevo control reproductivo"}
            </Typography>
            ...
            <Grid container spacing={2.5}>
              ...
              <Grid size={12}>
                <Button
                  ...
                  onClick={modoEdicion ? actualizarControlReproductivo : registrarControlReproductivo}
                  sx={botonRegistroInventarioSx}
                >
                  {modoEdicion ? "Guardar cambios" : "Registrar control reproductivo"}
                </Button>
              </Grid>
            </Grid>
          </CardContent>
                      ...
                      >
                        ...
                        <TableCell sx={{ maxWidth: 220, verticalAlign: "top" }}>
                          <CeldaObservacionConHistorial
                            ...
                            etapaLabel="Control reproductivo"
      ...
      {seleccionado && (
        <div style={{ marginTop: "20px", display: "flex", gap: "15px" }}>
          ...
          <Button
            variant="contained"
            color="error"
            onClick={() => eliminarControlReproductivo(seleccionado.fi_id ?? seleccionado.id)}
          >
            Eliminar registro
          </Button>
