...
export default function Reproductores() {
  ...
  const activarEdicion = () => {
    ...
    setFormData({
      ...
      cantidad_total: String(seleccionado.cantidad_total ?? seleccionado.cantidad ?? seleccionado.fn_cantidad ?? ""),
  ...
  const totalCantidad = registros.reduce(
    (acc, r) => acc + Number(r.cantidad_total ?? r.cantidad ?? r.fn_cantidad ?? 0),
    0,
  );
  ...
  return (
    <div style={{ padding: "25px" }}>
      ...
      <Paper sx={{ p: 2, mb: 3, backgroundColor: "#E3F2FD", boxShadow: 2 }}>
        ...
        <Typography><b>Total organismos en reproductores:</b> {totalCantidad.toLocaleString("es-MX")}</Typography>
      </Paper>
                  ...
                  {rows.length === 0 ? (
                    ...
                    rows.map((l) => (
                      ...
                      >
                        ...
                        <TableCell>{formatNumber(l.cantidad_total ?? l.cantidad ?? l.fn_cantidad)}</TableCell>
