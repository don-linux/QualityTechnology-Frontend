...
## Información General
Este es el frontend principal del sistema QualityTechnology, construido con **React** (v18), **Material UI (MUI)** y **React Router**.

## Estructura de Directorios
- `src/index.js`: Punto de entrada de la aplicación.
- `src/App.js`: Archivo principal de rutas y configuración de layout.
- `src/components/`: Componentes de página y elementos reutilizables (ej. `Login.jsx`, `Usuarios.jsx`).
- `src/layout/`: Plantillas de diseño generales (ej. `CorporateLayout.js`).
- `src/utils/`: Funciones de utilidad y estilos globales (ej. `GlobalStyles.js`).
...
1. **Rutas**: Las rutas se definen en `src/App.js`. Use el `PrivateRoute` para proteger rutas que requieren autenticación.
...
5. **Nuevas Funcionalidades**:
    ...
    - Registrar la ruta en `src/App.js` dentro del `CorporateLayout` si requiere la barra lateral.
